...
public class CameraFollow : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            if (Vector3.Distance(new Vector3(player.transform.position.x, 0, player.transform.position.z), new Vector3(transform.position.x+5, 0, transform.position.z - 15)) > 1)
            {
                transform.position += (new Vector3(player.transform.position.x, 0, player.transform.position.z) - new Vector3(transform.position.x-5, 0, transform.position.z + 15)) * Time.deltaTime;
            }
        }
        else
        {
